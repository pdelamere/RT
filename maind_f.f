...
c      call pgrd_reg()
c      call pgrd_irr()
c      call psetup(nf,b0,bt,nu)
c      call grd6()
      call grd7()
c      call grd6_setup(nf,b0,bt,nu)
c      call cov_to_contra(bt,btmf) 
c      call fgrd_reg()
      call fsetup(nf,b0,bt)
      call f_init(b0,b1,b12,uf,uf2,nf,up,np,input_p) 
c----------------------------------------------------------------------
...
c----------------------------------------------------------------------
     ...
     x         access='sequential',form='unformatted')                  
      ...
      call assign('assign -F system -N ultrix f:' //'c.ugradu.dat')
      open(300,file='c.ugradu.dat',status='unknown',
     x         access='sequential',form='unformatted')
...
cc         call Neut_Den(nn,nnd,m)
cc         call charge_exchange(np,xp,vp,vp1,m,chex_rate,
cc     x                        input_p)
         
c         !Ionize cloud and calculate ion density
c         call Ionize2(np,vp,vp1,xp,m,input_p,up)
c         write(*,*) 'Ni_tot...',Ni_tot !/beta
c         write(*,*) ' '
c         write(*,*) 'nu....',nu(1,1,1)

cc         call get_interp_weights(xp)
cc         call update_np(np)

cc         call get_gradP(gradP,np,nf,m,etemp)

cc         call check_np(np,nf)
cc         call fix_b1z(b1)

c         !energy diagnostics
cc         call p_diag(up,np,vp,vn,Ep)
c         call Energy_diag(vp,uf,nf,b1,E,Evp,Euf,EB1,EB1x,EB1y,EB1z,EE,
c     x                    EeP,etemp,nu)

c         call curlB(b1,nf,np,aj)
c         call cov_to_contra(bt,btmf) 
c         call face_to_center(btmf,btc)       !interp bt to cell center
c         call extrapol_up(up,vp,vp1,np)    
c         call get_Ep(Ep,aj,np,nf,up,uf,btc,nu,gradP)
c         call get_vplus_vminus(Ep,btc,vp,vplus,vminus)
c         call improve_up(vp1,vplus,vminus,up,np)

c         call get_Ep(Ep,aj,np,nf,up,uf,btc,nu,gradP)
c         call get_vplus_vminus(Ep,btc,vp,vplus,vminus)
c         call get_vp_final(Ep,vp,vp1,vplus)

c         call move_ion_half(xp,vp,vp1)  !1/2 step ion move to n+1/2
c         call get_interp_weights(xp)
c         call update_np(np)             !np at n+1/2
c         call update_up(vp,np,up)       !up at n+1/2
c**********************************************************************
c SUBCYCLING LOOP!
c**********************************************************************
...
c         call trans_nf_2(nf,nf1,ufp1)
         ...
         call correct_B(b0,b1,b1p2,E,aj,up,uf,np,nf,nu)
         call f_update_tlev(uf,uf2,b1,b12,b1p2,bt,b0)
         ...
         call get_bndry_Eflux(uf,b1,E,nf)

 2     continue
c**********************************************************************

c         call move_ion_half(xp,vp,vp1)  !final ion move to n+1
cc         call get_interp_weights(xp)
cc         call update_np(np)
cc         call Neut_Den(nn,m)

cc         call get_chex_rate(np,nn,up,m,chex_rate)
c         write(170) chex_rate
c         write(*,*) ' '
c         write(*,*) 'chex_rate...',chex_rate

c         call get_np_at_sat(np,m,satnp)
c         write(175) satnp
c         write(*,*) ' '
c         write(*,*) 'np at sat...',satnp

c         call get_ui(uf,nf,up,np,ui)
         ...
         write(*,*) '  Normalized............',
     x                        (pup(1)+puf(1)+peb(1))/input_p(2),
     x                        (pup(2)+puf(2)+peb(2))/input_p(2),
     x                        (pup(3)+puf(3)+peb(3))/input_p(2)
