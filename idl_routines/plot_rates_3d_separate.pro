...
@gops
!p.multi=[0,1,1]
device,filename='case1_growth_rates.ps'
;ff = (2*4*300./500)
device,/inches,xsize=6.0,ysize=6.0,xoffset=1
f = f*1e3
s = s*1e3

plot,mass,f,psym=1,xrange=[0.5,17.5],/xsty,yrange=[0.004,0.015]*1e3,/ysty,$
...
device,filename='case1_lndv_profiles.ps'
device,/color
...
plot,tm,lnvz,linestyle=0,xtitle='t (s)',ytitle='ln(v!dz!n)',xrange=[50,500],$
   /xsty
wh = where((tm ge t1) and (tm le t2))
fit = poly_fit(tm(wh),lnvz(wh),1,sigma=sigma)
;oplot,tm,fit(0)+fit(1)*tm
...
legend,['mass ratio 1','2','4','6','8','10'],linestyle=[0,1,2,3,4,5],$
  /bottom,/right
