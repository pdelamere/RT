...
for i = 1,nfrm do begin 
   ...
   f_read_3d_m_32,rundir+'/mixed_1',i,mix
   f_read_3d_m_32,rundir+'/npall_1',i,np
   f_read_3d_m_32,rundir+'/np_b_1',i,npb
   f_read_3d_m_32,rundir+'/temp_p_1',i,ti
...
;   ti=reform(ti(*,1,*))
   b12d=reform((1.0*(b1(*,1,*,2))^2 + 1.0*(b1(*,1,*,0))^2) + 1.0*(b1(*,1,*,1))^2)
endfor
...
s =  double((np+npb)*ti)/double(np*mp + 17*npb*mp)^gamma




b12d = b12d/(2*!pi*4e-7)/1e-11
s2d = alog(reform(s(*,1,*)))
p2d = reform(p(*,1,*))
npt2d = reform(np(*,1,*))/1e6
npb2d = reform(npb(*,1,*))/1e6
ti2d = reform(ti(*,1,*))
...
img_arr=img(x1:x2,nz/2-z1:nz/2+z2)
...
sz = size(img(x1:x2,nz/2-z1:nz/2+z2))
consx =5
consz =consx
vx = congrid(reform(up(x1:x2,1,nz/2-z1:nz/2+z2,0)),sz(1)/consz,sz(2)/consz)
vz = congrid(reform(up(x1:x2,1,nz/2-z1:nz/2+z2,2)),sz(1)/consz,sz(2)/consz)
...
;plots,[x(xslc),x(xslc)]/1e3,[!y.crange(0),!y.crange(1)],/data,color=fsc_color("white")
;plots,[!x.crange(0),!x.crange(1)],[z(zslc),z(zslc)]/1e3,/data,color=fsc_color("white")


img_cont,pimg_arr,xx/1e3,zz/1e3,nfrm
...
img_cont,tiimg_arr,xx/1e3,zz/1e3,nfrm
...
img_cont,npt_arr,xx/1e3,zz/1e3,nfrm
...
img_cont,npb_arr,xx/1e3,zz/1e3,nfrm
...
img_cont,simg_arr,xx/1e3,zz/1e3,nfrm
...
colorbar,minrange=mins2d,maxrange=mxs2d,format='(f5.2)',$
          position=[!y.window(0), !x.window(1)+0.01, !y.window(1), $
                    !x.window(1)+0.03],ncolors=255,/vertical,/right,$
          title='log(P/rho^gamma)',charsize=1.2,divisions=5,minor = 0,yaxis = 1
...
;plot,z/1e3,reform(up(xslc,1,*,0)),xrange=[-30,30],/xsty,$
;   yrange=[-max(up(xslc,1,*,0)),max(up(xslc,1,*,0))],/ysty
;oplot,z/1e3,reform(up(xslc,1,*,0)),color=fsc_color('red')
;;oplot,z/1e3,reform(up(xslc,1,*,1)),color=fsc_color('red')
;oplot,z/1e3,reform(up(xslc,1,*,2)),color=fsc_color('green')
plot,z/1e3,reform(b1(xslc,1,*,0))/1e-9,xrange=[-30,30],/xsty,$
   yrange=[-max(b1(xslc,1,*,0)/1e-9),max(b1(xslc,1,*,1)/1e-9)],$
   xtitle='x',ytitle='B (nT)'
oplot,z/1e3,reform(b1(xslc,1,*,0))/1e-9,color=fsc_color('red')

;oplot,z/1e3,reform(b1(xslc,1,*,1)),color=fsc_color('red')
oplot,z/1e3,reform(b1(xslc,1,*,2))/1e-9,color=fsc_color('green')
oplot,z/1e3,reform(b1(xslc,1,*,1))/1e-9,color=fsc_color('yellow')
oplot,z/1e3,10.0*reform(np(xslc,1,*))/max(np(xslc,1,*))
oplot,z/1e3,10.0*reform(npb(xslc,1,*))/max(npb(xslc,1,*))
oplot,z/1e3,reform(ti(xslc,1,*))/max(ti(xslc,1,*)),color=fsc_color('blue')
;plot,z/1e3,reform(ti(xslc,*)),xrange=[-30,30],/xsty
;plot,z/1e3,reform(mix(xslc,1,*)),xrange=[-30,30],/xsty

;window,2

;plot,x/1e3,reform(up(*,1,zslc,0)),$
;   yrange=[-max(up(*,1,zslc,0)),max(up(*,1,zslc,0))],/ysty
;oplot,x/1e3,reform(up(*,1,zslc,0)),color=fsc_color('red')
;;oplot,z/1e3,reform(up(*,1,zslc,1)),color=fsc_color('red')
;oplot,x/1e3,reform(up(*,1,zslc,2)),color=fsc_color('green')
plot,x/1e3,reform(b1(*,1,zslc,0))/1e-9,$
   yrange=[-max(b1(*,1,zslc,0)/1e-9),max(b1(*,1,zslc,1)/1e-9)],/xsty,$
   xtitle='z',ytitle='B (nT)'
oplot,x/1e3,reform(b1(*,1,zslc,0))/1e-9+reform(b1(*,1,zslc,2))/1e-9,color=fsc_color('red')

;oplot,z/1e3,reform(b1(*,1,zslc,1)),color=fsc_color('red')
oplot,x/1e3,reform(b1(*,1,zslc,2))/1e-9,color=fsc_color('green')
oplot,x/1e3,reform(b1(*,1,zslc,1))/1e-9,color=fsc_color('yellow')
oplot,x/1e3,reform(ti(*,1,zslc))/max(ti(*,1,zslc)),color=fsc_color('blue')


plot,x/1e3,1.0*reform(np(*,1,zslc))/max(np(*,1,zslc))
oplot,x/1e3,1.0*reform(npb(*,1,zslc))/max(npb(*,1,zslc))
;plot,x/1e3,reform(np(*,zslc))
;plot,x/1e3,reform(ti(*,zslc))
;plot,x/1e3,reform(mix(*,1,zslc))
