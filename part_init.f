...
c----------------------------------------------------------------------
      ...
      real press_tot 
      real*8 npb(nz),npt(nz)  !need to establish analytical density profile
      real*8 vth_bal(nz)      !thermal velocity necessary for pressure balance
      ...
      press_tot = np_bottom*0.5*m_bottom*vth_bottom**2

      npb(:) = np_bottom*((1.0-tanh((qz(:)-qz(nz/2))/(Lo)))/2.0) 
      npt(:) = np_top*((1.0+tanh((qz(:)-qz(nz/2))/(Lo)))/2.0) 


      where(npt(:) .gt. 0.0) 
      vth_bal(:) = sqrt((press_tot - npb(:)*0.5*m_bottom*vth_bottom**2 - 
     x       (1-f_proton_top)*npt(:)*0.5*m_heavy*mproton*vth_heavy**2)/
     x           (0.5*mproton*(f_proton_top)*npt(:)))
      endwhere

      write(*,*) 'vth_bal...',vth_bal


c      nprat = np_bottom/np_top
c      Ni_tot_heavy = Ni_tot

c add sheath protons to bottom first
...
c add protons to top (based on desired density relative to top)

         Ni_tot_1 = Ni_tot + 1
         Ni_tot_p_t = nint(Ni_tot*(np_top/np_bottom)*f_proton_top)
         Ni_tot_h_t = nint(Ni_tot_p_t*(1-f_proton_top))
         ...
         do 49 l = Ni_tot_1,Ni_tot
            ...
            flg = 0
            do 22 while (flg .eq. 0)
               xp(l,3) = qz(1)+(1.0-pad_ranf())*(qz(nz)-qz(1))
c               rnd = ((1.0-tanh((xp(l,3)-qz(nz/2))/(Lo)))/2.0) !for bottom
               rnd = (1.0+tanh((xp(l,3)-qz(nz/2))/(Lo)))/2.0 !for top
               if (pad_ranf() .le. rnd) flg = 1
               np_t_flg(l) = 1
               m_arr(l) = mproton
               mrat(l) = 1.0
...
c add heavies to bottom

         Ni_tot_1 = Ni_tot+1
         Ni_tot = Ni_tot + Ni_tot_h_t
...
c----------------------------------------------------------------------
      SUBROUTINE RT_maxwl_mix(np,vp,vp1,xp,input_p,up,
     x     np_t_flg,np_b_flg)
c----------------------------------------------------------------------
      include 'incurv.h'
      ...
      npb(:) = np_bottom*((1.0-tanh((qz(:)-qz(nz/2))/(Lo)))/2.0) 
      npt(:) = np_top !*((1.0+tanh((qz(:)-qz(nz/2))/(Lo)))/2.0) !uniform

      np_tot = npb+npt

      !bottom = 1 = variable density fixed temp
      !top = 2 = fixed density variable temp

c      where(npt(:) .gt. 0.0) 
c      vth_bal(:) = sqrt((press_tot - npb(:)*0.5*m_bottom*vth_bottom**2 - 
c     x       (1-f_proton_top)*npt(:)*0.5*m_heavy*mproton*vth_heavy**2)/
c     x           (0.5*mproton*(f_proton_top)*npt(:)))
c      endwhere

      vth_bal(1) = vth_top

c      do k = 2,nz 
c         grad_np = (npb(k)-npb(k-1))
c         vth_bal(k) = sqrt((vth_bal(k-1))**2 - 
c     x        grad_np*(vth_bottom**2)/np_top)
c       write(*,*) 'vth_bal...',npt(k)*vth_bal(k)**2+npb(k)*vth_bottom**2
c      enddo


      do k = 2,nz 

         vth_bal(k) = sqrt((vth_bal(1))**2 - 
     x        (vth_bottom**2/np_top)*(npb(k)-npb(1)) + 
     x        2*sum(grav(1:k,3)*np_tot(1:k))*delz/np_top)
       write(*,*) 'vth_bal...',vth_bal(k),grav(k,3),k
      enddo
      stop

c      write(*,*) 'vth_bal...',vth_bal,grav(k,3),my_rank
...
c         Ni_tot_h_t = nint(Ni_tot_p_t*(1-f_proton_top))
