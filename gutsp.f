...
cc----------------------------------------------------------------------
c      SUBROUTINE get_Ep_new(Ep,aj,np,nf,up,uf,btc,nu,gradP)
cc----------------------------------------------------------------------
cCVD$F VECTOR
c      include 'incurv.h'
...
c      real ajc(nx,ny,nz,3),       !aj at cell center
c     x     upc(nx,ny,nz,3),       !up at cell center
c     x     ufc(nx,ny,nz,3)       !uf at cell center
cc     x     gradPc(nx,ny,nz,3)     !gradP at cell center

c      real aa(Ni_max,3), bb(Ni_max,3), cc(Ni_max,3)    !dummy vars for doing cross product
c      real ntot(Ni_max)                   !total plasma density
c      real fnf(Ni_max),fnp(Ni_max)                !fraction nf,np of total n
c      real aj3(Ni_max,3),up3(Ni_max,3),uf3(Ni_max,3),  !vector field values at Ba position
c     x     btc3(Ni_max,3),gradP3(Ni_max,3)
c      real np_at_Ba(Ni_max)               !particle density at particle pos
c      real nf_at_Ba(Ni_max)
c      real ijkpp(Ni_max,3)
     

c      call face_to_center(aj,ajc)
c      call face_to_center(up,upc)
c      call face_to_center(uf,ufc)
cc      call face_to_center(gradP,gradPc)

c      ijkpp(:,1) = ijkp(:,1)+1
c      ijkpp(:,2) = ijkp(:,2)+1
c      ijkpp(:,3) = ijkp(:,3)+1

c      where(ijkpp(:,1) .ge. nx) ijkpp(:,1) = 2
c      where(ijkpp(:,2) .ge. nx) ijkpp(:,2) = 2
c      where(ijkpp(:,3) .ge. nx) ijkpp(:,3) = 2
...
cc         if (ip .ge. nx) ip = 2      !periodic boundary conditions
cc         if (jp .ge. ny) jp = 2      !periodic boundary conditions
cc         if (kp .ge. nz) kp = 2      !periodic boundary conditions

c         np_at_Ba(l) = np(i,j,k)*wght(l,1) + np(ip,j,k)*wght(l,2) + 
...
c         nf_at_Ba(l) = nf(i,j,k)*wght(l,1) + nf(ip,j,k)*wght(l,2) + 
...
c         ntot(l) = nf_at_Ba(l) + np_at_Ba(l)
c         fnf(l) = nf_at_Ba(l)/ntot(l)
c         fnp(l) = np_at_Ba(l)/ntot(l)
c      enddo

c      do 10 l = 1,Ni_tot
c         do 15 m=1,3 
c            aj3(l,m) = ajc(i,j,k,m)*wght(l,1) + ajc(ip,j,k,m)*wght(l,2) 
c     x          + ajc(i,j,kp,m)*wght(l,3) + ajc(ip,j,kp,m)*wght(l,4)
c     x          + ajc(i,jp,k,m)*wght(l,5) + ajc(ip,jp,k,m)*wght(l,6)
c     x          + ajc(i,jp,kp,m)*wght(l,7) + ajc(ip,jp,kp,m)*wght(l,8)

c            up3(l,m) = upc(i,j,k,m)*wght(l,1) + upc(ip,j,k,m)*wght(l,2) 
c     x          + upc(i,j,kp,m)*wght(l,3) + upc(ip,j,kp,m)*wght(l,4)
c     x          + upc(i,jp,k,m)*wght(l,5) + upc(ip,jp,k,m)*wght(l,6)
c     x          + upc(i,jp,kp,m)*wght(l,7) + upc(ip,jp,kp,m)*wght(l,8)

c            uf3(l,m) = ufc(i,j,k,m)*wght(l,1) + ufc(ip,j,k,m)*wght(l,2) 
...
cc            gradP3(l,m) = gradPc(i,j,k,m)*wght(l,1) 
cc     x                + gradPc(ip,j,k,m)*wght(l,2) 
cc     x                + gradPc(i,j,kp,m)*wght(l,3) 
cc     x                + gradPc(ip,j,kp,m)*wght(l,4)
cc     x                + gradPc(i,jp,k,m)*wght(l,5) 
cc     x                + gradPc(ip,jp,k,m)*wght(l,6)
cc     x                + gradPc(i,jp,kp,m)*wght(l,7) 
cc     x                + gradPc(ip,jp,kp,m)*wght(l,8)

c 15         continue

c         do 20 m=1,3
c            aa(l,m) = aj3(l,m) - fnp(l)*up3(l,m)  
c     x                     - fnf(l)*uf3(l,m)
c            bb(l,m) = btc3(l,m)                   
c 20         continue

c         cc(l,1) = aa(l,2)*bb(l,3) - aa(l,3)*bb(l,2)    !do cross product
c         cc(l,2) = aa(l,3)*bb(l,1) - aa(l,1)*bb(l,3)
c         cc(l,3) = aa(l,1)*bb(l,2) - aa(l,2)*bb(l,1)

c         do 30 m=1,3
c            Ep(l,m) = cc(l,m) + nu(i,j,k)*fnf(l)*(uf3(l,m)-up3(l,m)) 
cc     x                - gradP3(l,m) 
c     x                + nuei*aj3(l,m) 
cc                     + etar(i,j,k,m)*aj3(l,m)
c            Ep(l,m) = Ep(l,m)*mrat(l) !O_to_Ba
c 30         continue
c 10         continue
...
c         fnp = np_at_Ba/ntot
     ...
     x          + upc(i,jp,kp,m)*wght(l,7) + upc(ip,jp,kp,m)*wght(l,8)
            ...
            grv(m) = grav(k,m)*wght(l,1) + grav(k,m)*wght(l,2) 
     x          + grav(kp,m)*wght(l,3) + grav(kp,m)*wght(l,4)
     x          + grav(k,m)*wght(l,5) + grav(k,m)*wght(l,6)
     x          + grav(kp,m)*wght(l,7) + grav(kp,m)*wght(l,8)
...
cc      print *,'outflow particles...',Ni_tot,Ni_tot_in,Ni_tot-Ni_tot_in
...
c Tao BC (constant number of particles)
      
      do l = 1,Ni_tot 
         if (xp(l,3) .le. qz(1)) then
c            flg = 0
c            do 40 while (flg .eq. 0)
c               v = (400*pad_ranf())-200
...
c            vp(l,1) = vsw*(tanh((qz(1)-qz(nz/2))/(Lo)))+vx 
c            vp(l,2) = vy        !*sin(phi)*sin(theta)
            vp(l,3) = -vp(l,3)        !*cos(theta)
...
c            vp(l,1) = vsw*(tanh((qz(nz)-qz(nz/2))/(Lo)))+vx 
c            vp(l,2) = vy        !*sin(phi)*sin(theta)
            vp(l,3) = -vp(l,3)        !*cos(theta)
...
c      where(xp(1:Ni_tot,3) .le. qz(1))
c     x     in_bounds(1:Ni_tot) = .false.

c      Ni_tot_in = count(in_bounds)

cc      print *,'Ni_tot in_bounds...',Ni_tot_in
...
cc      print *,'Ni_tot in_bounds...',Ni_tot

ccc periodic boundary conditions
cc      where (xp(:,3) .ge. qz(nz))
cc         ijkp(:,3) = 1
cc         wquad(:,3) = 0.0
cc         xp(:,3) = qz(1) + ( xp(:,3) - qz(nz) )
cc         vp(:,1) = -vp(:,1)
cc      endwhere
...
c      where(xp(1:Ni_tot,3) .ge. qz(nz))
c     x     in_bounds(1:Ni_tot) = .false.

c      Ni_tot_in = count(in_bounds)
cc      print *,'Ni_tot_in top...',Ni_tot_in
...
ccc periodic boundary conditions
cc      where (xp(:,3) .le. qz(1)) 
cc         ijkp(:,3) = nz
cc         wquad(:,3) = -1.0
cc         xp(:,3) = qz(nz) - (qz(1) - xp(:,3))
c         vp(:,1) = -vp(:,1)
cc      endwhere
...
c               write(*,*) 'chex...',l,chex_tau,chex_pro
               vp(l,:) = 0.0
               mrat(l) = 1.0/m_pu
               m_arr(l) = mproton*m_pu
            endif
