c----------------------------------------------------------------------
      SUBROUTINE f_update_tlev(uf,uf2,b1,b12,b1p2,bt,b0)
...
c----------------------------------------------------------------------
     ...
     x     b0(nz)
      ...
      do 10 i=1,nx
         do 10 j=1,ny
            do 10 k=1,nz
               bt(i,j,k,1) = b1p2(i,j,k,1)
               bt(i,j,k,2) = b1p2(i,j,k,2)
               bt(i,j,k,3) = b1p2(i,j,k,3) + b0(k)
               do 10 m=1,3
      ...
      do 10 i=2,nx-1   
         do 10 j=2,ny-1
            do 10 k=2,nz-1
               ...
               ntot(1) = nf(k) + 0.5*(np(i,j,k)+np(ip,j,k))
               ntot(2) = nf(k) + 0.5*(np(i,j,k)+np(i,jp,k))
               ntot(3) = nf(k) + 0.5*(np(i,j,k)+np(i,j,kp))
      ...
      do 10 i=1,nx
         do 10 j=1,ny
            do 10 k=1,nz
               ...
               bth(i,j,k,1) = b1h(i,j,k,1)
               bth(i,j,k,2) = b1h(i,j,k,2)
               bth(i,j,k,3) = b1h(i,j,k,3) + b0(k)
 10            continue

      call cov_to_contra(bth,btmfh)
      call curlB(b1h,nf,np,ajh)

      call get_Ef(Ef,ajh,np,nf,up,uf,btmfh,nu,ugradu,delta_t)
...
c----------------------------------------------------------------------
 ...
 10              continue
      ...
      call get_Ef(Ef,aj,np,nf,up,ufp1,btmf,nu,ugradu,delta_t)
...
c----------------------------------------------------------------------
      SUBROUTINE get_E(E,bt,btmf,aj,up,uf,uf2,np,nf,nu)
...
c----------------------------------------------------------------------
     ...
     x     nu(nx,ny,nz)
      ...
      call get_E(E,bt,btmf,aj,up,uf,uf2,np,nf,nu)  !E at time level m 
...
c----------------------------------------------------------------------
      SUBROUTINE get_Ep1(E,b0,b1,b1p2,aj,up,uf,np,nf,nu)
      ...
      do 5 i=1,nx
         do 5 j=1,ny
            do 5 k=1,nz
               ...
               btp1(i,j,k,3) = b0(k) + 0.5*(b1p2(i,j,k,3)+b1(i,j,k,3))
...
c----------------------------------------------------------------------
      SUBROUTINE correct_B(b0,b1,b1p2,E,aj,up,uf,np,nf,nu)
...
c----------------------------------------------------------------------
     ...
     x     nu(nx,ny,nz)
      ...
      call get_Ep1(E,b0,b1,b1p2,aj,up,uf,np,nf,nu)  !E at time level m 
